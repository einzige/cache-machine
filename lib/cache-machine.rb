          ...
          def hook_cache_machine_on association_id
            ...
            when ActiveRecord::Reflection::AssociationReflection
              if reflection.macro == :has_and_belongs_to_many
                ...
                reflection.klass.after_save     { puts "called cache reset on #{target_class.name} - #{send reflection.primary_key_name} - #{target_class.find_by_id(send reflection.primary_key_name)}"; target_class.find_by_id(send reflection.primary_key_name).try(:delete_cache_of, association_id) }
