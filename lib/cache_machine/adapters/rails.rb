  ...
  module Adapters
    ...
    class Rails < CacheMachine::Adapter
      ...
      def reverse_association_ids(target, resource, association)
        ::Rails.cache.fetch(get_reverse_map_key(target, association)) do
      ...
      def append_id_to_reverse_map(target, association, id)
        key = get_reverse_map_key(target, association)
        ::Rails.cache.write(key, (::Rails.cache.read(key) || []) | [id])
      end
